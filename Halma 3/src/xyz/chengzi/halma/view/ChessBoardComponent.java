...
public class ChessBoardComponent extends JComponent implements Listenable<InputListener>, GameListener {
    ...
    private boolean isInJumpingState=false;
        ...
        for (int row = 0; row < dimension; row++) {
            for (int col = 0; col < dimension; col++) {
                gridComponents[row][col] = new SquareComponent(gridSize,
                        (row + col) % 2 == 0 ? BOARD_COLOR_1 : BOARD_COLOR_2,row,col);
    ...
    public void setChessAtGrid(ChessBoardLocation location, Color color) {
        ...
        getGridAt(location).add(new ChessComponent(color,isInJumpingState));
    }
    ...
    public void setInJumpingState(boolean inJumpingState) {
        isInJumpingState = inJumpingState;
    }

    public boolean isInJumpingState() {
        return isInJumpingState;
    }
